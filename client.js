...
let edit_object = function (out, mog, currow, nid) {
    ...
    let save_object = function () {
        waterfall([
            ajax_post_first_in_async_waterfall("setobject", {database: objdatabase, source: editor.getValue()}),
            function (x) {
                if (out) {
                    waterfall([
...
let dispquery = function (tree, node, query) {
    waterfall([
        ...
        function (x) {
            let wq = create_window("query", query + ' - ' + desktop.settings.nodesdb + ':' + tree.getPath(node.id.replace('#', '')));
            let gq =wq.attachGrid();
            let header = '';
            let colsorting = '';
            let filter = '';
