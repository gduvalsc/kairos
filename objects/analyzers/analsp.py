...
class UserObject(dict):
    def __init__(self):
        object = {
            ...
            "contextrules": [
                ...
                {"action": self.areq, "regexp": r'^(.*)$', "context": "req"},
                {"action": self.amod, "regexp": r'^Module: (.+)$|^(.+)$', "context": "mod"},
            ...
            "outcontextrules": [
                ...
                {"action": self.genstate('wev'), "regexp": r'Foreground Wait Events +(for DB| DB\/Inst):', "scope": "DBORAWEV"},
    ...
    def begin(self, a):
        ...
        a.getfloat=lambda f,x: a.tof(re.search(r'(^[-+]?[0-9,]*\.?[0-9]+([eE][-+]?[0-9]+)?$|^N/A$|^#+$)',x[a.fields[f][0]:a.fields[f][1]].lstrip().rstrip()).group(1))
        a.setContext('')
